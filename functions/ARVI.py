...
class ARVI():
        ...
        return [
            {
                ...
                'required': True,
                'displayName': "Raster",
                'description': "The multi-band input raster containing red, blue, and infrared bands."
            },
            {
                'name': 'red',
                'dataType': 'numeric',
    ...
    def getConfiguration(self, **scalars):
        ...
        blue = int(scalars.get('blue', 2))
        ir = int(scalars.get('ir', 3))
    ...
    def updateRasterInfo(self, **kwargs):
        ...
        if self.applyScaling:
            kwargs['output_info']['statistics'] = ({'minimum': 0.0, 'maximum': 200}, )
    ...
    def updatePixels(self, tlc, shape, props, **pixelBlocks):
        ...
        R = (np.array(inBlock[0], dtype='f4'))/10000            # need to convert band values to 0-1
        B = (np.array(inBlock[1], dtype='f4'))/10000
        NIR = (np.array(inBlock[2], dtype='f4'))/10000

        np.seterr(divide='ignore')
        # ARVI = (NIR - (R - 1 * (B - R))) / (NIR + (R - 1 * (B - R)))
        br = np.subtract(B, R)
        firstBlock = (NIR - (R - 1 * br)) / (NIR + (R - 1 * br))    # compute ARVI
        secondBlock = np.where(np.isinf(firstBlock), -1, firstBlock)   # take care of any divide by zero errors
        outBlock = np.where(np.bitwise_or(secondBlock < -1, secondBlock > 1), 0, secondBlock) #neutralize extreme values from water bodies and clouds
        if self.applyScaling:
